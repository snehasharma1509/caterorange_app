...
const Body = ({ isSidenavOpen }) => {
  ...
  const [showMap, setShowMap] = useState(false);
  const [address, setAddress] = useState('abcd, 500081');
  const [showCorporate, setShowCorporate] = useState(true); // To toggle between corporate and events
  const navigate = useNavigate(); // To navigate to different sections
      ...
      {!showMap ? (
        <div>
          ...
          <div className="flex justify-center gap-4 mb-6">
            <button
              className={`py-2 px-4 rounded-full font-semibold text-sm ${showCorporate ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-700'}`}
              onClick={() => setShowCorporate(true)}
            >
              Corporate
            </button>
            <button
              className="py-2 px-4 rounded-full font-semibold text-sm bg-gray-300 text-gray-700"
        ...
        <MapAddressSelector onAddressSelect={handleChangeAddress} />
      )}
    </div>
  );
};
